...
| `make all-generators` | Runs the code, CRD, swagger, and CLI/CRD-reference generators together (`codegen`, `generate-crds`, `generate-swagger-doc`, `generate-cli-docs`, `generate-crd-ref-docs`); run `make generate-webhooks` separately when webhook markers change. |
