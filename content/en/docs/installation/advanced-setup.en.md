...
**This guide covers resource limits, TLS, and router/executor tuning for running Fission on production systems with heavy workloads.**
...
the workload goes up and down in different time slots and it's not realistic to give a fixed number of replicas.
...
Router supports annotations so the LoadBalancer created in your cloud provider can be customized to your needs.
...
You can deploy the router as DaemonSet by setting `router.deployAsDaemonSet` to `true` when installing Fission with helm, to distribute requests across all nodes for better workload distribution and lower latency.
...
when using helm to deploy Fission 1.7.0.
Or, you can change the environment variable `DISPLAY_ACCESS_LOG` of the router deployment to `"true"`.
...
Storing in memory means it can't retrieve that information back if the executor pod gets deleted or restarted.
So when a new executor starts, it detects and removes the kubernetes objects created by the deleted executor to rebuild the resources map.
However, this approach may affect live requests to function pods, since it terminates/deletes all existing pods when the executor starts.

After 1.7.0, we use annotations to store all necessary information for resource adoption.
An experimental feature has also been added to the codebase that lets the executor adopt existing orphaned kubernetes objects.
Once the adoption feature is enabled, a new executor first tries to adopt all resources created by the old executor, instead of removing them like before.

There are no silver bullets: this adoption feature increases the overall startup time of the executor if there are huge amounts of resources in the cluster.
Also, requests to functions that have no function pods yet will fail while the executor is still booting up.
The executor waits at most 30 seconds for resource adoption, after which it starts the API service.

You can enable this feature by specifying `executor.adoptExistingResources` as `true` when installing/upgrading Fission 1.7.0 with `helm`.

```bash
--set executor.adoptExistingResources=true
```

Or, you can enable it by modifying the executor deployment's environment variable.
