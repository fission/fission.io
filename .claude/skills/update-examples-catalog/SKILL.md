...
## Facet filters (Trigger / Level)

The page filters examples by **Language** (from the group `language` field), **Trigger**, and **Level**.
Trigger and Level are derived from tags via a curated `TAG_FACETS` map at the top of the `<script>` in `content/en/examples/_index.html`.
When you add an example whose tag should be filterable (e.g. a new `timer`/`cron` trigger, or a broker like `pulsar`), add that tag to the matching value's `tags` array in `TAG_FACETS`.
Unmapped tags still render as searchable card pills — they just won't have a filter chip.
Since `examples.json` is regenerated by `tools/examples.py`, keep the facet map in the page (not the data), so it survives regen.
